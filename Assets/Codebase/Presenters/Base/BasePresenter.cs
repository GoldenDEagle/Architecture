...
namespace Assets.Codebase.Presenters.Base
{
    public abstract class BasePresenter : IPresenter, IDisposable
    {
        ...
        /// </summary>
        protected ViewId CorrespondingView = ViewId.None;
        ...
        {
            GameplayModel.ActiveViewId.Where(activeView => activeView == CorrespondingView).Subscribe(_ => CreateView()).AddTo(CompositeDisposable);
            GameplayModel.OnViewClosed.Where(activeView => activeView == CorrespondingView).Subscribe(_ => CloseView()).AddTo(CompositeDisposable);
        }
        ...
        {
            _isViewActive = true;
            var view = ServiceLocator.Container.Single<IViewCreatorService>().CreateView(CorrespondingView);
            view.Init(this);
        }
